...
class Person {
  ...
  void buyCar(Car car) {
    if (moneyLeft >= car.price) {
      ownedCars.add(car);
      moneyLeft -= car.price;
      print('Great choice, $name! You just bought a ${car.name} for \${car.price}. You now have \$$moneyLeft left.');
    } else {
  ...
  void sellCar(Car car) {
    if (ownedCars.contains(car)) {
      ...
      moneyLeft += car.price;
      print('You sold your ${car.name} for \${car.price}. Now you have \$$moneyLeft in your pocket.');
    } else {
...
void main() {
  ...
  while (true) {
    ...
    if (choice == 1) {
      ...
      print('Thanks for visiting the Car Buying and Selling Platform, $userName! Drive safe!');
      break;
    } else {
